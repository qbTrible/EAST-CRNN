...
class merge(nn.Module):
    ...
    def forward(self, x, x1, x2, x3):
        ...
        y = self.relu1(self.bn1(self.conv1(y)))  # 1 320 32 32
        ...
        y = self.relu3(self.bn3(self.conv3(y)))  # 1 160 64 64
        ...
        y = self.relu5(self.bn5(self.conv5(y)))  # 1 128 128 128
...
class output(nn.Module):
    ...
    def forward(self, x):
        inside_score = self.conv1(x)  # 1 1 128 128
        side_v_code = self.conv2(x)  # 1 2 128 128
        side_v_coord = self.conv3(x)  # 1 4 128 128
        east_detect = torch.cat((inside_score, side_v_code, side_v_coord), 1)
        return east_detect  # 1 7 128 128
